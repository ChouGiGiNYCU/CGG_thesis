...
int main(int argc,char* argv[]){
    ...
    string fully_combined_file = argv[13]; // 原本的方法做疊加，Extra/Payload都做punc
    string Enhanced_file = argv[14]; // 新的結構 一個被punc過的Extra 會跟 兩個payload bits做結合
    cout << "##############################" << "\n";
    ...
    cout << "Superposition_origin_file : " << fully_combined_file << "\n";
    cout << "Superpostion_Payload_Extra_NewStructure_pos_file : " << Enhanced_file << "\n";
    cout << "##############################" << "\n";
    // ========== define H_combine ==========
    ...
    Superposition_origin = Read_Extra2Pyalod_CSVfile(fully_combined_file);
    ...
    Superpostion_Payload_Extra_NewStructure = Read_NewStrcuture_CSVfile(Enhanced_file);
    ...
    double SNR = SNR_min;
    // length((Payload info) + length(Extra info))/ length(Payload CodeWord) 
    ...
    while(SNR <= SNR_max){
        ...
        while(payload_frame_error < frame_error_lowwer_bound){
            ...
            // Payload xor Extra with Enhanced Structure (Extra ^ Payload1 ^ Payload2)
        ...
        printf("PayLoad - SNR : %.2f  | FER : %.5e  | BER : %.5e(%d) |  Error frame : %.0f\n",SNR,payload_frame_error/frame_count,payload_bit_error_count[iteration_limit-1]/((double)PayLoad_H.n*frame_count),it,payload_frame_error);
        printf("Extra   - SNR : %.2f  | FER : %.5e  | BER : %.5e(%d) |  Error frame : %.0f | BER_num : %.0f\n",SNR,extra_frame_error/frame_count,extra_bit_error_count[iteration_limit-1]/((double)Extra_H.n*frame_count),it,extra_frame_error,extra_bit_error_count[iteration_limit-1]);
